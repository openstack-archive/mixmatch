...
function configure_mixmatch {
    ...
    iniset $NOVA_CONF neutron url "$MIXMATCH_URI/network"
...
function register_mixmatch {
    if [ "$REGISTER_MIXMATCH" == "true" ]; then
        ...
        get_or_create_endpoint \
            "volume" \
            "$REGION_NAME" \
            "$MIXMATCH_URI/volume/v1/\$(project_id)s" \
            "$MIXMATCH_URI/volume/v1/\$(project_id)s" \
            "$MIXMATCH_URI/volume/v1/\$(project_id)s"
        ...
        get_or_create_endpoint \
            "volumev2" \
            "$REGION_NAME" \
            "$MIXMATCH_URI/volume/v2/\$(project_id)s" \
            "$MIXMATCH_URI/volume/v2/\$(project_id)s" \
            "$MIXMATCH_URI/volume/v2/\$(project_id)s"
        ...
        get_or_create_endpoint \
            "volumev3" \
            "$REGION_NAME" \
            "$MIXMATCH_URI/volume/v3/\$(project_id)s" \
            "$MIXMATCH_URI/volume/v3/\$(project_id)s" \
            "$MIXMATCH_URI/volume/v3/\$(project_id)s"
