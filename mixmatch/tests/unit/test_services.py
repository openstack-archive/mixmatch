...
class TestServices(testcase.TestCase):
    ...
    def test_aggregate_key(self):
        ...
        response = json.loads(services.aggregate(
            samples.unaggregated['image']['v2']['unpaged'],
            'images',
            'image',
        ))
        self.assertEqual(
            len(samples.responses['image']['v2']['unpaged']['images']),
            len(response['images'])
        )

        # Aggregate 'volumes'
        response = json.loads(services.aggregate(
            samples.unaggregated['volume']['v2']['unpaged'],
            'volumes',
            'volume'
        ))
        self.assertEqual(
            len(samples.responses['volume']['v2']['unpaged']['volumes']),
    ...
    def test_aggregate_sort_images_ascending(self):
        ...
        smallest_img = min(
            samples.responses['image']['v2']['unpaged']['images'],
