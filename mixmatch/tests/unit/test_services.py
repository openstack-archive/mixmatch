...
VOLUMES = {
    'default': Response(json.dumps(
        samples.two_response['/volume/v2/id/volumes/detail'][0]
    )),
    'sp1': Response(json.dumps(
        samples.two_response['/volume/v2/id/volumes/detail'][1]
    ))
}

IMAGES = {
    'default': Response(json.dumps(
        samples.two_response['/image/v2/images'][0]
    )),
    'sp1': Response(json.dumps(
        samples.two_response['/image/v2/images'][1]
    ))
}

SMALLEST_IMAGE = min(
    samples.one_response['/image/v2/images']['images'],
    key=lambda d: d['size']
)['id']
EARLIEST_IMAGE = min(
    samples.one_response['/image/v2/images']['images'],
