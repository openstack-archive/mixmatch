...
def getproxy_url(base_url):
    """Return request protocol and IP address of proxy by parsing base_url"""
    if not base_url:
        return None, None
    return urlparse(base_url).scheme, urlparse(base_url).netloc.split(':')[0]
...
class RequestHandler(object):
    ...
    def __init__(self, method, path, headers):
        ...
        self.proxy_url = getproxy_url(request.base_url)
    ...
    def _do_request_on(self, sp, project_id=None):
        ...
        if self.details.service == 'volume':
            services.update_href(resp, self.proxy_url, local_project_id)
