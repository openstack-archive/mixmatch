...
def getproxy_ip(auth_url):
    """Return the IP address of the proxy by parsing auth_url"""
    if auth_url:
        return urlparse(auth_url).netloc.split(':')[0]
    return None
...
class RequestDetails(object):
    def __init__(self, method, orig_path, headers):
        ...
        self.proxy_ip = getproxy_ip(CONF.auth.auth_url)
