...
def _trim_project_id(orig_path):
    """Removes the project_id in the path."""
    path = str(orig_path).split('/')
    tail_variable = utils.safe_get(path, -1)
...
def _trim_version(orig_path):
    """Removes the version in the path."""
    path = str(orig_path).split('/')
    tail_variable = utils.safe_get(path, -1)

    if tail_variable in ['v1', 'v2', 'v3']:
        orig_path, version = os.path.split(orig_path)
...
def _trim_endpoint(endpoint):
    ...
    endpoint = list(urlparse.urlparse(endpoint))  # type: urlparse.ParseResult
    ...
    return urlparse.urlunparse(endpoint)
